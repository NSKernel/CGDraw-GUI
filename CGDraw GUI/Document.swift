    ...
    override func makeWindowControllers() {
        ...
        if let windowController =
            storyboard.instantiateController(
                withIdentifier: NSStoryboard.SceneIdentifier("CGDraw GUI Window Controller")) as? NSWindowController {
            addWindowController(windowController)
            
            // Set the view controller's represented object as your document.
            if let contentVC = windowController.contentViewController as? NSSplitViewController {
                contentVC.splitViewItems[0].viewController.representedObject = self
                contentVC.representedObject = self
                contentViewController = contentVC.splitViewItems[0].viewController as? ViewController
                let codeViewController = contentVC.splitViewItems[0].viewController as? ViewController
                let windowC = windowController as! WindowController
                if (codeViewController!.codeView.string != "") {
                    windowC.runButton.isEnabled = true
                    windowC.actionToolBar.setEnabled(false, forSegment: 5)
                    windowC.statusTextField.stringValue = "Update is required"
                }
            }
        }
    }
